"""
Module 5: 5D Extended Kalman Filter (EKF) Dead-Reckoning Core with Dynamic Bias Estimation & ZUPT.
Estimates state vector: [East (m), North (m), Velocity (m/s), Heading (rad), Yaw Bias (rad/s)].
Enforces strict zero-leakage GNSS outage isolation.
"""
...
@dataclass
class EKFResult:
    points: List[EKFStatePoint]
    dataframe: pd.DataFrame
    outage_start_t: float
    outage_end_t: float
    outage_duration_sec: float

def propagate_ekf_dead_reckoning(
    df_v: pd.DataFrame,
    ...
    """
    Executes 5D EKF state propagation:
    - State: x = [E, N, V, psi, b_z]
    - Process Model: Kinematic integration driven by longitudinal acceleration and raw yaw rate
    - Measurement Model: Sensor velocity update and Zero-Velocity Update (ZUPT) during stationary windows
    """
    dt_default = 0.1
    ...
    # Initial State Vector: [E, N, V, psi, b_z]
    ...
    for i in range(n_samples):
        ...
        if i > 0:
            ...
            # Update 1: Vehicle Speed Sensor Measurement
            z_v = float(v_slice["velocity_m_s"].iloc[i]) if "velocity_m_s" in v_slice.columns else float(v_slice["Velocity (km/hr)"].iloc[i]) / 3.6
            ...
            # Update 2: Zero-Velocity Update (ZUPT) when stationary
            if z_v < 0.05:
